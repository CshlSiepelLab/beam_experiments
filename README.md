...
The snakemake pipelines should be run from within an environment with `beam_sup` installed via pip, since many functions are called in the pipeline from that supplementary package. The pipelines will not run without `beam_sup` installed.
