...
for i, (topology, sims) in enumerate(topology_groups.items()):
    ...
    avg_df = (
        topology_df.groupby("Threshold")[["precision", "recall"]].mean().reset_index()
    )

    avg_machina_precision = np.nanmean(
        [machina_precisions[sim_names.tolist().index(sim)] for sim in sims]
    )
    avg_machina_recall = np.nanmean(
        [machina_recalls[sim_names.tolist().index(sim)] for sim in sims]
    )
    avg_metient_precision = np.nanmean(
        [metient_precisions[sim_names.tolist().index(sim)] for sim in sims]
    )
    avg_metient_recall = np.nanmean(
        [metient_recalls[sim_names.tolist().index(sim)] for sim in sims]
    )
    avg_random_precision = np.nanmean(
        [random_precisions[sim_names.tolist().index(sim)] for sim in sims]
    )
    avg_random_recall = np.nanmean(
        [random_recalls[sim_names.tolist().index(sim)] for sim in sims]
    )
    avg_consensus_precision = np.nanmean(
        [consensus_precisions[sim_names.tolist().index(sim)] for sim in sims]
    )
    avg_consensus_recall = np.nanmean(
        [consensus_recalls[sim_names.tolist().index(sim)] for sim in sims]
    )
    avg_parsimony_precision = np.nanmean(
        [parsimony_precisions[sim_names.tolist().index(sim)] for sim in sims]
    )
    avg_parsimony_recall = np.nanmean(
        [parsimony_recalls[sim_names.tolist().index(sim)] for sim in sims]
    )
    ...
    if i == len(topologies) - 1:
        ax.legend(
            bbox_to_anchor=(1.05, 0.5), loc="upper left", fontsize=14, edgecolor="none"
        )
