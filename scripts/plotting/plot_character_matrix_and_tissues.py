...
def main(barcode_file, tissue_file, primary_tissue, outfile):
    ...
    unique_tissues = sorted(
        set(tissue for tissues in tissue_df["tissues"] for tissue in tissues.split(","))
    )
    unique_tissues = [primary_tissue] + [
        tissue for tissue in unique_tissues if tissue != primary_tissue
    ]
    num_tissues = len(unique_tissues)
    start_value = 1000000
    for i, tissue in enumerate(unique_tissues):
        tissue_df[tissue] = tissue_df["tissues"].apply(
            lambda x: start_value + i if tissue in x.split(",") else 0
        )
    ...
    extended_palette = generate_extended_palette(
        len(unique_values) - len(non_zero_tissues)
    )
    tissue_colors = [
        "darkgrey",
        "blue",
    ...
    subset_tissue_colors = [
        tissue_colors[i]
        for i in range(num_tissues)
        if tissue_df[unique_tissues[i]].sum() > 0
    ]
