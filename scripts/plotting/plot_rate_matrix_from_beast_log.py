...
tissue_rate_col_names = [
    name for name in df.columns if name.startswith("tissueSubstModelLogger")
]
tissues = list(
    set(
        [
...
sample = np.random.randint(
    0, 1000
)  # if using a single sample, then draw a random sample from the end of the posterior samples
print(sample)
...
for source in tissues:
    for recipient in tissues:
        if source == recipient:
            continue
        rate = df[f"tissueSubstModelLogger.relGeoRate_{source}_{recipient}"].iloc[
            -sample
        ]  # to use a single sample
