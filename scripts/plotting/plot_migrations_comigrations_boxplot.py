...
melted_data = pd.melt(
    ...
    id_vars=["name"],
    value_vars=[
        "parsimony_migrations",
        "parsimony_comigrations",
        "machina_migrations",
        "machina_comigrations",
        "metient_migrations",
        "metient_comigrations",
        "beam_migrations",
        "beam_comigrations",
    ],
    var_name="category",
...
# Rename methods
melted_data["group"] = melted_data["group"].replace(
    {
        "parsimony": "Parsimony",
        "machina": "Machina",
        "metient": "Metient",
...
melted_data["type"] = melted_data["type"].replace(
    {"migrations": "Migrations", "comigrations": "Co-migrations"}
)
...
# Show the plot
plt.legend(
    title="", bbox_to_anchor=(1.0, 0.6), loc="upper left", fontsize=24, frameon=False
)
plt.tight_layout()
