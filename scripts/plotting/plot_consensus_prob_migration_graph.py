...
all_tissues = sorted(
    list(
        set([value for node in graph_dict.keys() for value in node.split("_")[0:2]])
        - {primary_tissue}
    )
)
...
dot = nx.nx_pydot.to_pydot(G)
dot.write_pdf(
    f"{outdir}/threshold_{consensus_probability_threshold}_migration_graph.pdf"
)
...
dot = nx.nx_pydot.to_pydot(G)
dot.write_pdf(
    f"{outdir}/threshold_{consensus_probability_threshold}_migration_graph_collapsed_numbered.pdf"
)
