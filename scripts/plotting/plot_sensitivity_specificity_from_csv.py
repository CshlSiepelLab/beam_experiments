...
df["beam"] = df.apply(
    lambda row: (
        "yes"
        if row["beam_bf"] > min_bf and row["beam_bf"] > row["min_bf_diff"]
        else "no"
    ),
    axis=1,
)
...
df = df.fillna(
    "no"
)  # assumes those that did not converge are not supporting reseeding. This is a safe assumption since the null hypothesis is no reseeding.

methods = [
    name
    for name in df.columns
    if name not in ["sim_name", "true", "beam_bf", "min_bf_diff", "information"]
]
sensitivity = []
specificity = []
...
print(
    f"{len(sims_needing_particles)} sims need more particles to decrease the minimum Bayes factor difference threshold."
)
