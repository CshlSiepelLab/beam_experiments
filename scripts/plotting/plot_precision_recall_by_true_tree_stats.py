...
# testing
file = "/grid/siepel/home_norepl/staklins/bayesian_phylogenetic_metastasis/results/variable_migration_and_mutation_rates_repeat_origin_scaling_implemented_10_25_24_data_from_8_19_24/precision_recall_variable_rates/precision_recall_vars_all.pkl"
...
for column in ['migration_count', 'comigration_count', 'num_multiedges']:
    ...
    for i, bin_label in enumerate(sorted(unique_bins, key=lambda x: (int(x.split('-')[0]), int(x.split('-')[1])))):
        ...
        avg_metient_precision = np.nanmean([metient_precisions[sim_names.tolist().index(sim)] for sim in bin_sims])
        avg_metient_recall = np.nanmean([metient_recalls[sim_names.tolist().index(sim)] for sim in bin_sims])
        avg_random_precision = np.nanmean([random_precisions[sim_names.tolist().index(sim)] for sim in bin_sims])
        avg_random_recall = np.nanmean([random_recalls[sim_names.tolist().index(sim)] for sim in bin_sims])
        avg_consensus_precision = np.nanmean([consensus_precisions[sim_names.tolist().index(sim)] for sim in bin_sims])
    ...
    plt.tight_layout()
    outfile = f"{outdir}/precision_recall_by_{column}_bin.pdf"
...
for column in ['met_to_met', 'reseeding', 'clonality']:
    ...
    for i, value in enumerate(unique_values):
        ...
        avg_metient_precision = np.nanmean([metient_precisions[sim_names.tolist().index(sim)] for sim in value_sims])
        avg_metient_recall = np.nanmean([metient_recalls[sim_names.tolist().index(sim)] for sim in value_sims])
        avg_random_precision = np.nanmean([random_precisions[sim_names.tolist().index(sim)] for sim in value_sims])
        avg_random_recall = np.nanmean([random_recalls[sim_names.tolist().index(sim)] for sim in value_sims])
        avg_consensus_precision = np.nanmean([consensus_precisions[sim_names.tolist().index(sim)] for sim in value_sims])
    ...
    fig.suptitle(title, fontsize=fs)
    plt.tight_layout()
    outfile = f"{outdir}/precision_recall_by_{column}.pdf"
    plt.savefig(outfile, bbox_inches="tight")
    plt.close()
