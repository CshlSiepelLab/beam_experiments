...
data = pd.merge(data, tip_count_data, left_on=['sim', 'downsample_threshold'], right_on=['sim_name', 'downsampling_threshold'])

# Replace the f1_score calculation with the custom function
data['f1_score'] = data.apply(lambda row: calculate_f1_score(row['precision'], row['recall']), axis=1)

# get the migration counts
data['migration_count'] = data.apply(lambda row: get_number_of_migrations_from_graph_dict(row['thresh_counts']), axis=1)
...
color_map = {threshold: DEFAULT_COLORS[i % len(DEFAULT_COLORS)] for i, threshold in enumerate(unique_thresholds)}


# plot tip count by migration count
plt.figure(figsize=(10, 6))
for sim, group in data.groupby('sim'):
    plt.plot(group['tip_count'], group['migration_count'], color='lightgrey', linestyle='-', linewidth=1, zorder=1)
    scatter = plt.scatter(group['tip_count'], group['migration_count'], c=group['downsample_threshold'].map(color_map), marker='o', zorder=2)
...
plt.plot(data['tip_count'], m * data['tip_count'] + b, color='black', linestyle='--', linewidth=2)

# Create a legend for the colors
handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=color, markersize=10) for threshold, color in color_map.items()]
labels = [f'{threshold}' for threshold in color_map.keys()]
plt.legend(handles, labels, title="Downsampling\ndistance\nthreshold", bbox_to_anchor=(1.05, 1), loc='upper left', frameon=False, fontsize=16, title_fontsize=16)

# Add the Pearson correlation coefficient to the title
plt.text(0.65, 0.95, f"Pearson r={pearson_corr:.2f}", fontsize=16, transform=plt.gca().transAxes, verticalalignment='top')
plt.text(0.05, 0.95, f"y={m:.2f}x + {b:.2f}", fontsize=16, transform=plt.gca().transAxes, verticalalignment='top')
plt.ylim(0,70)
