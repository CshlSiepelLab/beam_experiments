...
fs=22
plt.figure(figsize=(14, 8))
strip_plot = sns.stripplot(x='mouse', y='num_asvs', data=data, color='grey', size=7.5, jitter=True, alpha=0.5)
box_plot = sns.boxplot(x='mouse', y='num_asvs', data=data, palette=DEFAULT_COLORS, showcaps=True, boxprops=dict(facecolor="none", edgecolor="black"), whiskerprops=dict(color="black"), fliersize=0)
plt.axhline(max_tips, color='red', linestyle='--')
...
for i, mouse in enumerate(unique_mice):
    ...
    for j in range(mouse_data.shape[0]):
        if mouse_data.iloc[j]['num_asvs'] >= max_tips:
            plt.text(i, mouse_data.iloc[j]['num_asvs'] + 5, mouse_data.iloc[j]['cp'], color='black', ha="center", fontsize=fs)
