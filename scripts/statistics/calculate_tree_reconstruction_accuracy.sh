...
for true_tree_file in $(find $data_indir -type f -name "cell_tree*.nwk"); do
    ...
    vine_trees_file=${results_indir}/vine/${simname}/vine_tissue_trees.nex
    # vine_trees_file=${results_indir}/vine/${simname}/vine_trees.nwk   # For trees without migration
    ...
    python $script $true_tree_file $laml_tree_file $cassiopeia_tree_file $beam_trees_file $vine_trees_file $outfile $pathfinder_tree_file
