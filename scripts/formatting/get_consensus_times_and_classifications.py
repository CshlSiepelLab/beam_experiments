...
# outprefix = "/grid/siepel/home_norepl/staklins/bayesian_phylogenetic_metastasis/results/quinn_2021_lung_cancer_data_2_21_25/beam_gtr/5k/58/58"
...
expected_migration_times_all = {k: v for k, v in expected_migration_times_all.items() if len(v) / total_graphs > consensus_threshold}
...
expected_migration_times_all = {k: np.mean(v) for k, v in expected_migration_times_all.items()}
expected_migration_times_no_multiedge = {k: np.mean(v) for k, v in expected_migration_times_no_multiedge.items()}


# write out the expected migration times
outname = os.path.basename(outprefix)
with open(outprefix + '_expected_migration_times_no_multiedge.csv', 'w') as file:
...
with open(outprefix + '_expected_migration_times_all.csv', 'w') as file:
...
with open(outprefix + '_consensus_classifications.csv', 'w') as file:
