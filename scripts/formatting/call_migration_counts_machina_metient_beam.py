...
def getMigrationComigrationCountsFromNewick(newick):
    ...
    for node in tree.traverse():
        ...
        if parent_tissue != child_tissue:
            ...
            if (
                edge not in comigration_edges_already_checked
            ):  # All unique edges are considered a co-migration
                comigration_count += 1
...
if os.path.isfile(machina):
    ...
    with open(machina, "r") as f:
        for line in f.readlines():
            ...
            if (
                not machina_met_to_met
                and source != primaryTissue
                and recipient != primaryTissue
            ):
    ...
    all_tissues = set(
        [
            tissue
            for migration in machina_migrations.keys()
            for tissue in migration.split("_")
        ]
    )
    ...
    machina_comigration_count = sum(
        1 for key, value in machina_migrations.items() if value > 0
    )  # All unique edges are considered a co-migration
else:
...
if os.path.isfile(metient):
        ...
        for key, value in graph_dict.items():
            for k, v in value.items():
                if v == 0:
                    ...
                    comigration_count += (
                        1  # All unique edges are considered a co-migration
                    )
    ...
    metient_comigration_count = sum(all_comigration_counts) / len(
        all_comigration_counts
    )
...
if os.path.isfile(parsimony):
    ...
    for newick in parsimony_solutions:
        mig_count, comig_count, met_met, reseeding = (
...
if os.path.isfile(beam):
    ...
    for tree in trees:
        ...
        mig_count, comig_count, met_met, reseeding = (
            getMigrationComigrationCountsFromNewick(bnewick)
        )
