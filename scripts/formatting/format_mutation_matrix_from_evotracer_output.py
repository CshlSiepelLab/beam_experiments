...
for group in grouped_site_matrix:
    ...
    print(f"Site {site_num} has {count_above_one} occurences of more than 1 mutation in the same ASV.")

collapsed_matrix = aggregate_site_matrix(grouped_site_matrix)
joint_mut_matrix, joint_mut_dict = replace_multiple_mutations(collapsed_matrix, mut_dict)

output_path = evotracer_mut_matrix_path.split(".")[0] + "_reformatted_tidetree.tsv"
joint_mut_matrix.to_csv(output_path, sep = '\t')

output_dict_path = evotracer_mut_matrix_path.split(".")[0] + "_reformatted_tidetree_dictionary.tsv"
