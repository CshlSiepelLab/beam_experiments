...
tissue_dict = {}
print(
    "CP,metastatic_confined,metastatic_mono_seeding,metastatic_parallel_seeding,metastatic_re_seeding,primary_confined,primary_mono_seeding,primary_parallel_seeding,primary_re_seeding"
)
with open(sys.argv[1], "r") as infile:
    lines = infile.readlines()
    lines.append("END")
    for l in lines:
        ...
        if cp != cur_cp or l == ["END"]:
            ...
            for asv in leaf_list:
                if "XXX" not in asv:
                    parent_list = []
                    cur_parent = list(parent_dict.keys())[
                        get_index(list(parent_dict.values()), asv)
                    ]
                    ...
                    while (
                        ...
                        cur_parent = list(parent_dict.keys())[
                            get_index(list(parent_dict.values()), cur_parent)
                        ]
                        parent_list.append(cur_parent)
                    parent_tissues = []
