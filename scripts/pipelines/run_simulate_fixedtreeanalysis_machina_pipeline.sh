...
pipeline_run_name="moresims_compare_beast_machina_fixedtreeanalysis_variableSampleSize_variableMigrationRate_2_5_24"
...
do
    ...
    do
        for ((j=1; j<=${num_trees}; j++))
        do
        # Simulate data
        conda activate simulate
        ./scripts/sim_wrapper.sh --design RANDOM --out sim${i} --sites 10 --mutrate 1.0 --samples ${size} --migrationrate ${rate} --migration inputs/test_migration_prob_matrix.csv
        ...
        echo -e "sim${i}\t${size}\t${rate}" >> ${inputs_file}
        i=$(( i+1 ))
        done
    done
done
