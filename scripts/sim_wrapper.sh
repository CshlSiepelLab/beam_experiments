...
if [[ $# -eq 0 ]] ; then
    echo "Usage: sim_wrapper.sh --design <BC10v0 or TAPE or RANDOM> --sites <num_sites> --out <out_name> --mutrate <float|comma-sep list of floats> --samples <int> [--migration <filepath>] [--sites <int>]"
    exit 0
fi
...
while [[ "$#" -gt 0 ]]; do
    ...
    *) echo "Unknown parameter passed: $1"; echo "Usage: sim_wrapper.sh --design <BC10v0 or TAPE> --sites <num_sites> --out <out_name> --mutrate <float|comma-sep list of floats> --samples <int> [--migration <filepath>] [--sites <int>]"; exit 1 ;;
    esac
    shift
done
...
../barcode_simulator/scripts/simulator/simulator.py ${outprefix} ${DESIGN} ${NUM_SITES} ${MUTRATE} ${NUM_SAMPLES} ${MIGRATION_MATRIX}
